							...
							<div class="section" id="credit_summary">
								...
								<div style="width:850px;margin:auto;overflow:hidden">
									<div style="width:400px;float:left" class="font14">
										<xsl:if test="CommercialDelphi/CommDelphiScore!=''">
										    
										<div class="dataLabel">commercial delphi score</div>
										<div class="dataValue">
											<span>
												<xsl:value-of select="number(CommercialDelphi/CommDelphiScore)"/>
											...
											<xsl:variable name="score">
												<xsl:value-of select="number(CommercialDelphiHistory/CompanyHistory/CommDelphiScore)"/>
											</xsl:variable>
											<xsl:if test="CommercialDelphiHistory/CompanyHistory/CommDelphiScore!=''">
												<div class="subSectionInsert" style="width:300px;float:left">
													...
													<div
														...
														><xsl:value-of
														select="number(CommercialDelphiHistory/CompanyHistory/CommDelphiScore)"
														/> out of 100 </div>
														<div
												...
												<!--<div class="section" id="report_update_flag"><h2>Report update flag</h2>
												<div class="dataLabel" style="clear:both">Report update flag</div>
												<div class="dataValue">
													<xsl:choose>
														<xsl:when test="ScoringData/ReportUpdateFlag=0">
															No Update Available
														</xsl:when>
														<xsl:when test="ScoringData/ReportUpdateFlag=1">
															More Recent Accounts Filed
														</xsl:when>
														<xsl:when test="ScoringData/ReportUpdateFlag=2">
															Accounts Imbalanced
														</xsl:when>
														<xsl:when test="ScoringData/ReportUpdateFlag=3">
															Auditors Qualification Level 2
														</xsl:when>
														<xsl:when test="ScoringData/ReportUpdateFlag=4">
															Foreign Currency
														</xsl:when>
														
														<xsl:when test="/RawBureauData/IsPATHSViewReport = 1">
														    <xsl:value-of select="ScoringData/ReportUpdateFlag" />
														</xsl:when>
													</xsl:choose>
												</div>
											</div>-->
