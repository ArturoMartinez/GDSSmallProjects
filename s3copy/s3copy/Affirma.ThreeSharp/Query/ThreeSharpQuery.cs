...
namespace Affirma.ThreeSharp.Query
{
    ...
    public class ThreeSharpQuery : IThreeSharp
    {
        ...
        /// <summary>
        /// Changes a payment request configuration
        /// </summary>
        public PaymentChangeResponse PaymentChange(PaymentChangeRequest request)
        {
            return Invoke<PaymentChangeResponse>(request);
        }
        ...
        private void AddHeaders(WebRequest req, SortedList<string, string> headers)
        {
        ...
        private void AddHeaders(WebRequest webRequest, SortedList<string, string> headers, string prefix)
        {
            if (headers != null)
            {
                foreach (string key in headers.Keys)
                {
                    ...
                    {
                        webRequest.Headers.Add(prefix + key, headers[key]);
                    }
                }
            }
        }
        ...
        private void AddMetadataHeaders(WebRequest webRequest, SortedList<string, string> metadata)
        {
            ...
            if (httpWebRequest.Method == "PUT" || httpWebRequest.Method == "POST")
            {
                using (Stream httpWebRequestStream = httpWebRequest.GetRequestStream())
                {
                    if (request.DataStream != null)
                    {
                        ...
                        request.TransferInfo = new TransferInfo(request);
                        lock (this.statistics) { this.statistics.AddTransferInfo(request.TransferInfo); }       // All operations on this.statistics must be thread-safe
                        ...
                        while (true)
                        {
                            ...
                            request.TransferInfo.BytesTransferred += bytesRead;
                        }
        ...
        public TransferInfo GetTransferInfo(String id)
        {
            ...
            lock (this.statistics)
            {
                transfer = this.statistics.GetTransferInfo(id);
            }
            return transfer;
        }

        /// <summary>
        /// Returns an array of TransferInfo objects, which contain statistics about data transfer operations
        /// </summary>
        public TransferInfo[] GetTransferInfos()
        {
            ...
            lock (this.statistics)
            {
                transfers = this.statistics.GetTransferInfos();
            }
            return transfers;
        }
