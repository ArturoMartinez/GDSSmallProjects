 ...
 */
public class InfoUSAProcessor extends HTTPProcessor{
    
private static final Logger log = Logger.getLogger(NetConnectProcessor.class.getName());

    public InfoUSAProcessor(Socket clientSocket, Settings settings, Generator generator)
    {
    ...
    {
        log.info("Responding to InfoUSA request");
        ...
        XPathExpression expr = xpath.compile("/Request/companyname");

        log.fine("Extracting CompanyName");
        String strCompanyName = expr.evaluate(document);

        log.log(Level.INFO, "CompanyName = {0}", strCompanyName);
    ...
    protected void sendErrorResponse(String strMessage)
    {
        try
        {
            log.warning("Returning default response to InfoUSA request");
