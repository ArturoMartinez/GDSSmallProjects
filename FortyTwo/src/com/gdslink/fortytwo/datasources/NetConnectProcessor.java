 ...
 * @author simon.smith
 */
public class NetConnectProcessor extends HTTPProcessor
{
    private static final Logger log = Logger.getLogger(NetConnectProcessor.class.getName());

    public NetConnectProcessor(Socket clientSocket, Settings settings, Generator generator)
    {
    ...
    protected String getInputKey() throws Exception
    {
        ...
        XPathExpression expr = xpath.compile("/NetConnectRequest/Request/Products/CreditProfile/PrimaryApplicant/SSN");

        log.fine("Extracting SSN");
        String strSSN = expr.evaluate(document);

        log.log(Level.INFO, "SSN = {0}", strSSN);
